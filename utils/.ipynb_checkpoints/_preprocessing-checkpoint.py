...
def _preprocessing_dataset(ds, scale="minmax"):
    # output dim ~ X.shape (len(ds), 2, 1000)
...
def _preprocessing_scaling(X, y):    
    scaler = MinMaxScaler()
    X[:, 0, :] = scaler.fit_transform(X[:, 0, :].T).T
    X[:, 1, :] = scaler.fit_transform(X[:, 1, :].T).T
...
def _preprocessing_dataset_noising(ds, noise_scale, scale="minmax"):
    # output dim ~ X.shape (len(ds), 2, 1000)
    ...
    for idx in range(n_ds):
        traj = data[idx]
        traj += np.random.randn(*traj.shape) * noise_scale
        length = traj.shape[-1]
        padded_len = default_len - length
        traj = np.pad(traj, ((0, 0), (0, 0), (padded_len, 0)))
        X[idx] = traj[0]
...
def _get_dataloader(X, y, batch_size=64, shuffle=True):
...
def _preprocessing(ds, batch_size=32, shuffle=True, scale="minmax"):
    ...
    """
    X, y = _preprocessing_dataset(ds, scale)
    dataloader = _get_dataloader(X, y, batch_size, shuffle)
