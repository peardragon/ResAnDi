...
def _plot_confusion_matrix(ground_truth, model_results):
    ...
    plt.savefig(f"./figures/_model_results_confusion_plot.pdf")
    plt.show()



def _get_model_results(dataset=None, model_name="resnet18_8_b64_lr0.0001_noiseless", model_tag="", shuffle=False,
    ...
    model_path = f"./saved_models/{model_name}/checkpoint{model_tag}.pt"
...
def _model_confidence_plot(model_results, dataset):
    ...
    grouped_df = pd.DataFrame( df.groupby(['Exponent', "Truth Label"], observed=False)["Result"].sum()).reset_index(level=[0,1])
    df['range'] = df.groupby('Truth Label', observed=False)[['Exponent']].transform(lambda x: pd.cut(x, bins = exp_ranges))
    df["Exponent"] = df["range"].apply(lambda x: x.mid)
    
    
    predicted_counts = df.groupby(['Exponent', "Truth Label","Predicted Label"], observed=False)["Predicted Label"].count()
    total_counts = df.groupby(['Exponent', "Truth Label"], observed=False)["Truth Label"].count()
    ...
    plt.savefig("./figures/confidence_level.pdf")
